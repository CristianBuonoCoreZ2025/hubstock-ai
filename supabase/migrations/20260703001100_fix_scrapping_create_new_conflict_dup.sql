-- ============================================================================
-- Migracion: Evitar ON CONFLICT DO UPDATE duplicado en catalog_retail_links
-- y catalog_retail_snapshots.
--
-- Problema:
--   _tmp_scrapping_catalog puede tener multiples filas con el mismo
--   (retailer, external_ref) si el mismo producto aparece en varias URLs
--   de scrapping. El INSERT en catalog_retail_links con ON CONFLICT DO UPDATE
--   falla con error 21000 porque intenta afectar la misma fila 2 veces.
--
-- Cambios:
--   1. PASO 11: catalog_retail_links usa DISTINCT para evitar duplicados.
--   2. PASO 11: catalog_retail_snapshots usa DISTINCT para evitar snapshots
...
begin
  ...
  insert into public.catalog_products (
    ...
    default_reference_price, sort_order, active, source_system, source_product_url
  )
  ...
  -- FIX: DISTINCT evita duplicados de (retailer, external_ref) que causaban
  --      error 21000 en ON CONFLICT DO UPDATE.
  insert into public.catalog_retail_links (retailer, external_ref, catalog_product_id, updated_at)
  select distinct sc.retailer, sc.external_ref, sc.catalog_product_id, now()
  ...
  -- FIX: DISTINCT evita snapshots duplicados del mismo producto.
  ...
  v_image_pending := 0;  -- ahora la imagen se guarda automaticamente
