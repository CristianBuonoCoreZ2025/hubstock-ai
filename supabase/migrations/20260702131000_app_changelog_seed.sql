...
-- Cada version tiene una fecha ligeramente diferente para asegurar orden correcto.
...
insert into public.app_changelog (version, module, description, files_changed, author, tags, created_at) values
  ('1.0.001', 'scraping', 'Reparacion STOP en captura-cadenas-2: barridoApiPhase2Seal ahora propaga AbortSignal. Proteccion contra doble STOP. Reduccion de llamados duplicados.',
  ...
  ('1.0.002', 'diagnostico', 'Log de diagnostico global activable desde Configuracion. Sanitizacion de datos sensibles. Auto-limpieza por navegacion. Panel global en AppShell.',
  ...
  ('1.0.003', 'performance', 'Optimizacion carga inicial captura-cadenas-2: llamada combinada barridoApiInit que devuelve retails + runs en un solo request (~81% mas rapido).',
  ...
  ('1.0.004', 'diagnostico', 'Trazabilidad completa: sesiones por pagina, traceId, duplicados, lentos, errores. Traza frontend->backend->base en endpoints criticos.',
  ...
  ('1.0.005', 'diagnostico', 'Interceptor global de fetch para trazabilidad en toda la app. Captura automatica de todas las llamadas HTTP en todas las paginas.',
    array['src/lib/request-logger.ts', 'src/components/layout/AppShell.tsx'],
    'devin', array['feature', 'fetch-interceptor', 'diagnostico'],
    now() - interval '50 minutes'),

  ('1.0.006', 'instrumentacion', 'Instrumentacion de paginas principales (catalogo, inventario, consumo) y helper server-side withServerActionDiagnostic.',
  ...
  ('1.0.007', 'homologacion', 'Motor de homologacion paso 2: empty catalog guard para evitar ~35,600 busquedas de similitud innecesarias cuando catalog_products esta vacio.',
  ...
  ('1.0.008', 'homologacion', 'Creacion de productos nuevos desde scrapping via RPC atomico en Postgres. Reemplaza batches de 1,000 filas por funcion SQL unica. Mapeo de taxonomia Lider.',
  ...
  ('1.0.009', 'homologacion', 'Fix: evita duplicados por URL dentro del lote de pending_new. Si dos filas de scrapping comparten la misma URL, apuntan al mismo producto maestro.',
    array['supabase/migrations/20260702130000_fix_create_new_products_duplicate_url.sql', 'src/app/(app)/captura-cadenas-2/create-new-products-modal.tsx', 'src/lib/request-logger.ts'],
    'devin', array['fix', 'duplicados', 'homologacion', 'sql'],
    now() - interval '10 minutes'),

  ('1.0.010', 'versiones', 'Panel de versiones en Configuracion con historial de cambios tecnicos desde app_changelog. Registro estructurado de mejoras arquitectonicas.',
    array['src/app/(app)/settings/page.tsx', 'src/app/actions/changelog.ts', 'src/components/changelog-modal.tsx', 'supabase/migrations/20260702131000_app_changelog_seed.sql'],
