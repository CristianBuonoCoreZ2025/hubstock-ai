-- ============================================================================
-- Migracion: Evitar duplicados de PK en catalog_products al crear productos
-- desde scrapping_create_new_products_all.
--
-- Problema:
--   El INSERT en catalog_products (PASO 10) hace INNER JOIN con _tmp_candidates.
--   Si hay multiples filas de scrapping con el mismo name_norm+brand_norm+price
--   (ej: diferentes URLs del mismo producto en Jumbo), el JOIN genera multiples
--   filas con el mismo new_catalog_product_id, y la segunda falla con PK 23505.
--
-- Cambios:
--   1. PASO 10: Agrega DISTINCT ON (np.new_catalog_product_id) al INSERT
--      para insertar solo una fila por producto unico.
-- ============================================================================
...
begin
  ...
  -- FIX: DISTINCT ON evita duplicados de PK cuando _tmp_candidates tiene
  --      multiples filas con el mismo name_norm+brand_norm+price
  ...
  -- catalog_retail_snapshots AHORA incluye image_url
