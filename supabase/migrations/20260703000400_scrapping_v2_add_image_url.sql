-- ============================================================================
-- Migracion: Agregar image_url a scrapping_create_new_products_all v2.
--
-- Problema anterior:
--   La funcion SQL v2 NO copiaba image_url de scrapping a
--   catalog_retail_snapshots ni a catalog_product_media.
--   Cuando se borraba la tabla scrapping, las URLs de imagen se perdian.
--
-- Cambios:
--   1. _tmp_candidates ahora incluye image_url desde scrapping.
--   2. INSERT en catalog_retail_snapshots ahora incluye image_url.
--   3. NUEVO: INSERT en catalog_product_media con bucket_id='external'
--      (la URL externa se guarda como referencia; luego un proceso
--       batch descarga y reemplaza por storage interno).
--   4. El conteo v_image_pending pasa a 0 porque la imagen ya queda
--      referenciada, no "pendiente".
-- ============================================================================
